...
// Removes various APIs such as cryptography, DDE, RPC, and Windows Sockets.
#define WIN32_LEAN_AND_MEAN
// Defines both the above definitions as well as a ton more.
#define VC_EXTRALEAN
...
extern "C" void *malloc(unsigned int size) {
  return HeapAlloc(
    g_hHeap,
    HEAP_ZERO_MEMORY | HEAP_NO_SERIALIZE | HEAP_GENERATE_EXCEPTIONS,
    size);
}
...
void __cdecl operator delete(void *p) { HeapFree(g_hHeap, 1, p); }
...
void compress(uint32_t num, int rounds, char *target) {
  const char *lu = "0123456789ABCDEFGHIJKLMNPQRSTUVWXYZ";
  uint32_t index = 0;
  int i;
  for (i = 0; i < rounds; i++) {
    index = num % 0x23;
...
void getAuth(char *hostname, char *result) {
  ...
  char dos[8];
  char trs[8];
  compress((uint32_t)((buffer[3] << 24) + (buffer[2] << 16) + (buffer[1] << 8) +
                      buffer[0]),
           7, uno);
  compress((uint32_t)((buffer[7] << 24) + (buffer[6] << 16) + (buffer[5] << 8) +
                      buffer[4]),
           7, dos);
  compress((uint32_t)((buffer[11] << 24) + (buffer[10] << 16) +
                      (buffer[9] << 8) + buffer[8]),
           7, trs);
...
void getPK(char *ret) { // Blech char*
  static uint32_t majors[16] = {48335, 48334, 48333, 48332, 48331, 48330,
                                48329, 48328, 48327, 48326, 48325, 48324,
                                48323, 48322, 48321, 48320};
...
HANDLE AddResourceFont(char *ResID) {
  ...
  DWORD Count = 0;
  if (Resource) {
    HGLOBAL res = LoadResource(hInst, Resource);
    if (res)
      return AddFontMemResourceEx(
          LockResource(res), SizeofResource(hInst, Resource), NULL, &Count);
  }
  return nullptr;
}
...
int WINAPI winMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR pCmdLine,
                   int nCmdShow) {
  hInst = hInstance;
  ...
  hwnd = CreateWindowExA(WS_EX_LAYERED | WS_EX_COMPOSITED, wc.lpszClassName, "",
                         WS_VISIBLE | WS_SYSMENU | WS_POPUP, 100, 100, WINDOW_WIDTH, WINDOW_HEIGHT,
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  ...
  static HBRUSH hbBreak = CreateSolidBrush(0x00EFDE52);
  ...
  static RECT nullRect = {0, 0, WINDOW_WIDTH, 126};
  static char hostname[128];

  switch (msg) {
  case WM_COMMAND: {
    if (HIWORD(wParam) == EN_SETFOCUS &&
        (LOWORD(wParam) == 12 || LOWORD(wParam) == 13 ||
         LOWORD(wParam) == 14)) {
  ...
  case WM_CREATE: {
    ...
    // Background
    HWND icBg = CreateWindowA("STATIC", NULL, WS_CHILD | WS_VISIBLE, 0, 90, WINDOW_WIDTH,
                              170, hwnd, (HMENU)10, hInst, NULL);

    HWND icPINKIETIEM =
        CreateWindowA("STATIC", "", WS_VISIBLE | WS_CHILD, 110, 0, 122, 126,
                      hwnd, (HMENU)11, hInst, NULL);
    SetTimer(hwnd, 1, 80, NULL);

    HWND icSer = CreateWindowA(
        "EDIT", NULL, WS_TABSTOP | WS_VISIBLE | WS_CHILD | ES_RIGHT,
        WINDOW_WIDTH - 80 - 5, 189, 80, 16, hwnd, (HMENU)12, hInst, NULL);
    HWND icPK = CreateWindowA(
        "EDIT", NULL, WS_TABSTOP | WS_VISIBLE | WS_CHILD | ES_RIGHT,
        WINDOW_WIDTH - 125 - 5, 214, 127, 16, hwnd, (HMENU)13, hInst, NULL);
    HWND icAC = CreateWindowA(
        "EDIT", NULL, WS_TABSTOP | WS_VISIBLE | WS_CHILD | ES_RIGHT,
        WINDOW_WIDTH - 210 - 5, 239, 210, 16, hwnd, (HMENU)14, hInst, NULL);

    HFONT hfDefault =
        CreateFontA(16, 0, 0, 0, FW_DONTCARE, FALSE, FALSE, FALSE,
  ...
  case WM_PAINT: {
    ...
    case 260: {
      ...
      // BG
      HRGN rounded = CreateRoundRectRgn(cRect.left, cRect.top, cRect.right + 1, cRect.bottom - 89, 15, 15);
      FillRgn(ichdc, rounded, hbBg);
      ...
      if (dbhbm == NULL) {
        ...
        dbhbm = CreateCompatibleBitmap(ichdc, WINDOW_WIDTH, 170);
        HBITMAP hHBOld = (HBITMAP)SelectObject(dbdc, dbhbm);
        BitBlt(dbdc, 0, 0, WINDOW_WIDTH, 170, ichdc, 0, 0, SRCCOPY);
    ...
    case 126: {
      ...
      BitBlt(ichdc, 0, 0, WINDOW_WIDTH, 170, dbdc, 0, 0, SRCCOPY);
      ...
      BitBlt(icghdc, 0, 0, pinkBM.bmWidth, pinkBM.bmHeight, pmdc, 0, 0,
             SRCPAINT);
  ...
  case WM_TIMER: {
    ...
    RedrawWindow(hwnd, &nullRect, NULL,
